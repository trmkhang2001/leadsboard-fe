...
function LeadBoard({ data: initialData }) {
      ...
      <div className="bg-gray-900 rounded-lg p-4 mb-4 border border-gray-800">
        ...
        <div className="mb-2">
          <div className="flex items-center mb-1">
            ...
            <span className="text-white font-bold fs-3">
              Mục Tiêu Tổng: {data.totalTarget} Leads
            </span>
          </div>
          ...
          <div className="text-green-500 flex items-center mb-3 font-bold">
            ...
            Số leads đã đạt được: {data.achievedLeads}
          </div>
          ...
          {data.inputHistory.length > 0 ? (
            data.inputHistory.map((entry, index) => (
              <div key={index} className="text-white text-sm mb-2">
                {entry.timestamp}: Thêm {entry.value} leads (Tổng: {entry.total}
                )
              </div>
            ))
          ) : (
